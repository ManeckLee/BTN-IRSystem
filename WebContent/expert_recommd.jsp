...
<script type="text/javascript">
	function startrecommend() {
		...
		if(!userproblem=="")
		{
			xmlhttp.open("POST", "<%=basePath %>expert_recommend_ser?userproblem=" + userproblem + "&status=" + status, 
					true);
		}
		else
		{
		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) 
			{
				...
				var res = xmlhttp.responseXML.getElementsByTagName("length")[0].firstChild.data;
				if (res>0)
				{
					var row = res;//行
				    var col = 3;//列
				    ...
				    for(var i=0;i<row;i++)
				    {  
				    	expertname = xmlhttp.responseXML.getElementsByTagName("expertname")[i].firstChild.data;
				    		tabstr+="<tr>";  
				            
				            tabstr+="<td>";
				           	tabstr+="<a  ";
				           	tabstr+="  onclick=\"javascript:post('Servlet',{expert_name:'";
				           	tabstr+= expertname;
				           	tabstr+="', status:'browse'})\" >";
				           	tabstr+= expertname;
				           	tabstr+= "</a>";
				            tabstr+="</td>";
				            ...
				            tabstr+="<td>";  
				          	 tabstr+= xmlhttp.responseXML.getElementsByTagName("expert_company")[0].firstChild.data;
				            tabstr+="</td>";  
				            ...
				            tabstr+="<td>";  
				        	tabstr+= xmlhttp.responseXML.getElementsByTagName("basis")[i].firstChild.data;
				            tabstr+="</td>";  
					...
					document.getElementById("info").innerHTML = "没有查询到与该课题相关的专家";
				}
			}
...
<div class="container">
	...
	<div class="sixteen columns">
		<div class="headline no-margin"><h3>输入课题开始推荐</h3></div>
		<p>请在下面输入研究课题内容，我们将利用推荐出与该研究方向可能相关的专家列表，并给出推荐依据</p>
		<textarea id="userproblem"  rows="6" cols="150" style="resize:none" ></textarea>
		<input type="button" id="search"  onclick="startrecommend();"  value="开始推荐"/>

	</div>
	
	<div class="sixteen columns">
		<div class="headline no-margin"><h3>推荐结果：</h3></div>
